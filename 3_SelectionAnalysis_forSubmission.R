...
# This script will generate a plot corresponding to Supplemental Figure S3
...
quantile.function.lower = function(x) {quantile(x, c(0.025))}
quantile.function.upper = function(x) {quantile(x, c(0.975))}

#  Read in fitness data
##  Reading body size data into R
selection.data.github = getURL("https://raw.githubusercontent.com/asingh164/SexSpecificVarianceEfficacyOfSelection/master/FitnessDataNeprojectFinalSept2020.csv")
data.raw=read.csv(text = selection.data.github)
data=subset(data.raw, data.raw$n.total=="32") # Clean data out for any row where we counted less than 32 offspring data=subset(data, data$n.total=="32") # Clean data out for any row where we counted less than 32 offspring 
...
# A - First plot is selection differentials separately for each sex and population type 
...
selection.plot.all.seperate = ggplot(sel.diff.data.merge, aes(y = sel.diff, x = as.factor(ID))) + 
  ...
  geom_errorbar(aes(ymin = lower.ci, ymax = upper.ci, width = 0.0)) +
  theme_bw() + ylim(-1.1,1) +
...
hetsub.summary.table.by.sex$upper.ci = c(as.vector(quantile(female.sel.vector, c(0.975))), as.vector(quantile(male.sel.vector, c(0.975))), as.vector(quantile(both.sexes.sel.vector, c(0.975))))
hetsub.summary.table.by.sex$lower.ci = c(as.vector(quantile(female.sel.vector, c(0.025))), as.vector(quantile(male.sel.vector, c(0.025))), as.vector(quantile(both.sexes.sel.vector, c(0.025))))
